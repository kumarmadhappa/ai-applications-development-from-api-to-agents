from t2_llms_output_tuning._clients.zai_client import ZaiAIClient
from t2_llms_output_tuning._main import run

#Tasks copied from Open AI
# TODO 1: n — number of completions to generate per request. Default: 1
#  ⚠️ Note: NOT available in ZAI API
...
#  ⚠️ Note: Not availabe in ZAI API
...
run(
    client=ZaiAIClient(model_name='glm-5.1'),
    print_request=True, # Switch to False if you do not want to see the request in console
    print_only_content=False, # Switch to True if you want to see only content from response
    #temperature = 0.0, # Try changing to 0.0 and 1.0
    #top_p=0.1, # Try changing to 0.1 and 0.9
    #max_tokens=150, # Try changing to 50 and 2048
    #stop=["5"], # Try changing to ["5"] and None
    #response_format={"type": "json_object" } #Supports two formats:{ "type": "text" } plain text mode, { "type": "json_object" } JSON mode, returns valid JSON data.
    response_format={"type": "json_object", "json_object": {"name": "languages", "strict": True, "schema": {"type": "object", "properties": {"languages": {"type": "array", "items": {"type": "object", "properties": {"name": {"type": "string"}, "year": {"type": "integer"}}, "required": ["name", "year"], "additionalProperties": False}}}, "required": ["languages"], "additionalProperties": False}}}
